...
public class MyWebGraph {
	...
	public boolean addEdge(String s, String t) {
		if(vertexList.containsKey(s) && vertexList.containsKey(t)) {
			WebVertex vertex1= vertexList.get(s);
			return vertex1.addEdge(t);
		}
		return false;
	}
    ...
    public ArrayList<String> getEdgesInto(String v) {
		...
		for(String vertex : getVertices()){
			if (vertexList.get(vertex).links.contains(v)){
				edgesInto.add(vertex);
			}
		}
		return edgesInto;
    }
    ...
    public double getOutDegree(String url) {
